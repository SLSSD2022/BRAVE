...
//  analogWrite(CH4, Speed); 
  ...
  Serial.println(xMag_buf[Mag_index]);
  yMag_buf[Mag_index] = yMag;
  Serial.println(yMag_buf[Mag_index]);
  Mag_index = (Mag_index+1)%MAG_BUF_LEN;
  Serial.println(Mag_index);
  //キャリブレーション値を計算する
...
int xcenter_calculation() {
  //ソート用のバッファ
  static int sortxBuf[MAG_BUF_LEN];

  //ソート用バッファにデータをコピー
  Serial.println("------------buffer-----------------");
  for(int i=0; i<MAG_BUF_LEN; i++) {
    sortxBuf[i] = xMag_buf[i];
//    Serial.print(i);
//    Serial.print("th data:");
    Serial.println(xMag_buf[i]);
  }
  Serial.println("------------buffer-----------------");

  //クイックソートで並べ替える
  qsort(sortxBuf, MAG_BUF_LEN, sizeof(int), quicksortFunc);

//  Serial.println("------------Sortbuffer-----------------");
  for(int i=0; i<MAG_BUF_LEN; i++) {
//    Serial.print(i);
//    Serial.print("th data:");
//    Serial.println(xMag_buf[i]);
...
//  Serial.println("------------Sortbuffer-----------------");
  ...
  return (sortxBuf[0] + sortxBuf[(int)MAG_BUF_LEN-1])/2;
}

int ycenter_calculation() {
  //ソート用のバッファ
  static int sortyBuf[MAG_BUF_LEN];

  //ソート用バッファにデータをコピー
//  Serial.println("------------buffer-----------------");
  for(int i=0; i<MAG_BUF_LEN; i++) {
    sortyBuf[i] = yMag_buf[i];
//    Serial.print(i);
//    Serial.print("th data:");
//    Serial.println(yMag_buf[i]);
  }
//  Serial.println("------------buffer-----------------");

  //クイックソートで並べ替える
  qsort(sortyBuf, MAG_BUF_LEN, sizeof(int), quicksortFunc);
...
//  Serial.println("------------Sortbuffer-----------------");
  ...
  return (sortyBuf[0] + sortyBuf[(int)MAG_BUF_LEN-1])/2;
}
