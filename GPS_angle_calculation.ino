...
int   yMag  = 0;
int   zMag  = 0;
double Calib = 15; //キャリブレーション用定数(最初はセンサに書いてある矢印に対して微妙に0°がずれてるので、ローバーの進行方向と並行な向きの矢印が磁北（0°）になるよう調整）
double Calibx =-70;
double Caliby = 0;
float LatA = 35.713718, LongA =139.760162;  //目的地Aの緯度経度
...
// Serial.print("Rover's LONG="); Serial.println(gps.location.lng(), 6);
 ...
 Serial.println(degRtoA); //rvoverと目的地Aの真北を基準にした角度のずれ
...
// Serial.print("deg: The distance between Rover and A is= ");    
 ...
 //BMX055 磁気の読み取り
  ...
  if (x < degRtoA){
    ...
    if (0 < delta_theta && delta_theta < threshold){
